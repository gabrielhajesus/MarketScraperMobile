...
def inserirnobanco(item={}, produtos=produtos):
    ...
    if item['old_price_card'] == "":
        descontoo = 0
    else:
        descontoo = desconto(item['old_price_card'],
                             item['price_card'], item['loja'])
    if produtos.find_one(nome_de_busca) != None:
        ...
        if produtobanco['menor_preco'] > item['price_card']:
            produtos.update_one({'nome': item['name']}, {'$set': {"menor_preco": item['price_card'], "desconto": desconto(
                item['old_price_card'], item['price_card'], item['loja'])}})
        if item['loja'] == 'Kabum':
...
def definindocategoria(nomec):
    catHardware = ['processador', 'placa', 'memoria',
                   'SSD', 'fonte', 'fan', 'ddr3', 'ddr4']
    contadorHardware = 0
    catPeriferico = ['headset', 'mouse', 'teclado',
                     'gabinete', 'mousepad', 'mochila']
...
def desconto(old_price, new_price, loja):
    old_price = old_price.replace('R$', '').replace(
        '\xa0', '').replace('.', '').replace(',', '')
    new_price = new_price.replace('R$', '').replace(
        '\xa0', '').replace('.', '').replace(',', '')
    desconto = int((float(old_price) - float(new_price))
                   * 100 / float(old_price))
    return desconto
